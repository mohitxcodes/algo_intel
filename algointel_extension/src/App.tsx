...
const ANALYSIS = {
  ...
  spaceComplexity: "O(1)",
  bestTime: "O(n)",
  bestApproach: "Hash Map Lookup",
  summary:
    "Uses nested iteration to check all possible pairs in the array. While correct, this approach is inefficient for large inputs due to quadratic time complexity.",
  improvements: [
    "Replace nested loops with a HashMap for O(1) complement lookups",
    "Use a single-pass approach instead of double iteration",
    "Add early return when a valid pair is found",
    "Handle edge cases — empty array, single element, no valid pair",
    "Consider using TypeScript generics for type-safe inputs",
...
function Divider({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3 py-1">
      ...
      <span className="text-[10px] font-semibold text-text-muted uppercase tracking-[0.12em] whitespace-nowrap">{label}</span>
      <div className="flex-1 h-px bg-border" />
    </div>
  );
}
...
export default function App() {
  ...
  const d = ANALYSIS;
  const visible = showAll ? d.improvements : d.improvements.slice(0, 4);
      ...
      <header className="sticky top-0 z-50 backdrop-blur-xl bg-bg-primary/80 border-b border-border">
        <div className="flex items-center justify-between px-5 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[13px] font-bold tracking-tight bg-gradient-to-r from-text-primary to-accent bg-clip-text text-transparent">AlgoIntel</span>
          </div>
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-emerald/10 border border-emerald/20">
            ...
            <span className="text-[9px] font-semibold text-emerald">Active</span>
          </div>
        </div>
      </header>
      ...
      <div className="px-5 py-5 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="p-4 rounded-xl bg-bg-card border border-border space-y-2.5">
            ...
            <p className="font-mono text-xl font-bold text-text-primary">{d.timeComplexity}</p>
            <p className="text-[10px] text-text-muted leading-snug">Quadratic — nested iteration</p>
          </div>
          ...
          <div className="p-4 rounded-xl bg-bg-card border border-border space-y-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-semibold text-text-muted uppercase tracking-wider">Space</span>
            </div>
            <p className="font-mono text-xl font-bold text-text-primary">{d.spaceComplexity}</p>
            <p className="text-[10px] text-text-muted leading-snug">Constant — no extra memory</p>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3.5 p-4 rounded-xl bg-bg-card border border-border">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                <h4 className="text-[13px] font-semibold text-text-primary">{d.approach}</h4>
                <span className="font-mono text-[9px] font-bold px-1.5 py-0.5 rounded-md bg-red/10 text-red border border-red/20">
                  {d.timeComplexity}
                </span>
              </div>
              <p className="text-[12px] text-text-secondary leading-[1.7]">{d.summary}</p>
            </div>
          </div>

          {/* Best Possible */}
          <div className="flex items-center gap-3 p-3.5 rounded-xl bg-emerald/[0.05] border border-emerald/12">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#34d399" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
          ...
          <ul className="space-y-2">
            {visible.map((text, i) => (
              ...
              >
                ...
                <p className="text-[12.5px] text-text-secondary leading-[1.65]">{text}</p>
              </li>
            ))}
          </ul>
        ...
        <div className="pt-3 pb-1">
          ...
          <p className="text-[9px] text-text-muted/50 font-mono text-center">AlgoIntel v1.0</p>
        </div>
      </div>
    </div>
  );
